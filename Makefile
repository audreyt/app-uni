# This Makefile is for the App::Uni extension to perl.
#
...
#     ABSTRACT => q[Command-line utility to grep UnicodeData.txt]
...
TO_INST_PM = lib/App/Uni.pm

PM_TO_BLIB = lib/App/Uni.pm \
	blib/lib/App/Uni.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) \
	  bin/uni $(INST_MAN1DIR)/uni.$(MAN1EXT) 
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/App/Uni.pm $(INST_MAN3DIR)/App::Uni.$(MAN3EXT) 
...
$(INST_SCRIPT)/uni : bin/uni $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/uni
	$(CP) bin/uni $(INST_SCRIPT)/uni
	$(FIXIN) $(INST_SCRIPT)/uni
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/uni
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Command-line utility to grep UnicodeData.txt</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/App/Uni.pm blib/lib/App/Uni.pm 
